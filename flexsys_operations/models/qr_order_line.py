...
class FlexSysOrderLine(models.Model):
    ...
    order_id = fields.Many2one('qtcafe.qr.order', required=True, ondelete='cascade', index=True)
    ...
    preparation_state = fields.Selection([
        ...
        ('cancelled', 'Cancelled'),
    ], string='Preparation Status', default='new', required=True, index=True, tracking=True)
    unavailable_reason = fields.Selection([
        ('out_of_stock', 'Out of Stock'),
        ('equipment', 'Equipment Issue'),
        ('temporarily_stopped', 'Temporarily Stopped'),
        ('other', 'Other'),
    ], string='Unavailable Reason', tracking=True)
    kitchen_note = fields.Char(string='Kitchen Note', tracking=True)
