...
class QtCafeQrPrinterConfig(models.Model):
    _name = 'qtcafe.qr.printer.config'
    _description = 'QT Cafe QR Printer Settings'
    ...
    shop_name = fields.Char(string='Receipt Shop Name', default='QT Cafe')
    ...
    def action_test_printer(self):
        for config in self:
            config.print_text('%s\n%s\n\nTest Print OK\n' % (config.shop_name or 'QT Cafe', fields.Datetime.now()))
        return True
