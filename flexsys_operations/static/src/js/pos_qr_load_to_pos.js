...
patch(ControlButtons.prototype, {
    async onClickQtCafeQrOrders() {
        if (this.__qtcafeLoadingQrOrder) {
            return;
        }
        this.__qtcafeLoadingQrOrder = true;

        try {
            const configId = this.pos?.config?.id || false;

            const response = await rpc("/qtcafe/qr_orders/pending_orders", {
            ...
            const result = await rpc("/qtcafe/order/action", {
                order_id: order.id,
                action: "accept",
            });
            ...
            this.__qtcafeLoadingQrOrder = false;
        }
    },
});
