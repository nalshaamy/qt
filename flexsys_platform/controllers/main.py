...
class FlexSysPlatformController(http.Controller):
    ...
    @http.route('/flexsys/login', type='http', auth='public', website=True, methods=['GET', 'POST'], csrf=True)
    def login(self, **post):
    ...
    @http.route('/flexsys/logout', type='http', auth='public', website=True, csrf=False)
    def logout(self):
        session = self._current_session()
        if session:
            request.env['flexsys.system.log'].record(
    ...
    @http.route('/flexsys/context', type='http', auth='public', website=True, methods=['POST'], csrf=True)
