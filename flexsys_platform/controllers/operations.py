...
class FlexSysOperationsQrMenuController(http.Controller):
    ...
    def _public_brand_prefix(self):
        raw = request.env['ir.config_parameter'].sudo().get_param(
            'flexsys_platform.public_url_prefix', 'brand'
        )
    ...
    def customer_start(self, **kwargs):
        ...
        if not store_settings.is_open and not store_settings.allow_browse_when_closed:
            return self._render('flexsys_platform.qr_store_closed_page', {
                'store_settings': store_settings,
        ...
        return self._render('flexsys_platform.qr_customer_start_page', {
            'guest_url': guest_url,
            'login_url': login_url,
    ...
    def customer_phone_login(self, **post):
        ...
        return self._render('flexsys_platform.qr_phone_login_page', values)
    ...
    def customer_forgot_password(self, **post):
        ...
        return self._render('flexsys_platform.qr_forgot_password_page', values)
    ...
    def customer_phone_register(self, **post):
        ...
        return self._render('flexsys_platform.qr_phone_register_page', values)
    ...
    def menu(self, **kwargs):
        ...
        if branches and len(branches) > 1 and not selected_branch:
            return self._render('flexsys_platform.qr_branch_selector_page', {
                'branches': branches,
        ...
        if selected_branch and not selected_branch.operations_branch_is_open:
            return self._render('flexsys_platform.qr_branch_closed_page', {
        ...
        return self._render('flexsys_platform.qr_menu_page', values)

    @http.route(['/self-order/orders', '/qr-menu/my-orders'], type='http', auth='user', website=True)
    def my_orders(self, **kwargs):
        partner = self._registered_partner()
        if not partner:
            return request.redirect('/qr-menu/login?redirect=/self-order/orders')
        ...
        return self._render('flexsys_platform.qr_my_orders_page', {
            'orders': orders,
            'customer_partner': partner,
            'public_base_url': self._public_base_url(),
            'state_labels': {
    ...
    def manager_independent_login(self, **post):
        ...
        return self._render('flexsys_platform.qr_manager_login_page', values)
    ...
    def manager_dashboard(self, **kwargs):
        ...
        if not manager or not manager.can_view_dashboard:
            return request.redirect('/flexsys/login')
        return self._render('flexsys_platform.qr_manager_dashboard_page', {
            'store_settings': self._get_store_settings(),
    ...
    def cashier_dashboard(self, **kwargs):
        return self._render('flexsys_platform.cashier_dashboard_page', {})
    ...
    def kds_dashboard(self, **kwargs):
        return self._render('flexsys_platform.kds_dashboard_page', {})
    ...
    def customer_order_tracking(self, token, lang='ar', **kwargs):
        ...
        return self._render('flexsys_platform.customer_order_tracking_page', {
            'order': order,
    ...
    def create_order(self, lines=None, customer_name=None, customer_mobile=None, note=None, pos_config_id=None,
        ...
        try:
            service_env = request.env(context=dict(
                request.env.context,
                tracking_disable=True,
                mail_create_nosubscribe=True,
                mail_notrack=True,
            ))
            order = OrderService(service_env).create_self_order(
                lines=lines,
            ...
            _logger.exception(
                'Unexpected error while creating a Self Order (uid=%s, partner_id=%s, pos_config_id=%s)',
                request.env.uid, partner.id if partner else False, pos_config_id,
            )
            return {
                'success': False,
