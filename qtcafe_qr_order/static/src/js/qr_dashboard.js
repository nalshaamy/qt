...
function renderOrders(screen, orders) {
    const box = document.querySelector('#qtcafe-orders');
    if (!box) return;
    if (!orders.length) {
    ...
    box.querySelectorAll('button[data-action]').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            await rpc('/qtcafe/order/action', {order_id: id, action: btn.dataset.action});
            await loadOrders();
        });
    });
}
