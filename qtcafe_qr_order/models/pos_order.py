...
class PosOrder(models.Model):
    ...
    def _qtcafe_sync_qr_order_ready(self):
        """Mark linked QR orders as ready once the POS order is paid/done.
        ...
        for pos_order in self:
            ...
            try:
                qr_order.sudo().message_post(
                    body='QR Order marked Ready automatically after POS payment: %s' % (
    ...
    def _qtcafe_sync_qr_order_cancelled(self, force=False):
        """Mark linked QR orders as cancelled.
        ...
        for pos_order in self:
            ...
            try:
                qr_order.sudo().message_post(
                    body='QR Order marked Cancelled automatically from POS: %s' % (
