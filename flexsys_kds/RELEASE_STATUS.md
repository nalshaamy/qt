...
through v7.19.0 (see CHANGELOG.md for the full history), plus a naming
correction - confirmed live that "Copy Agent Key" never actually copied
anything to the clipboard, only revealing the key in a sticky
notification for manual selection, exactly as designed, but the name
itself was misleading. Given the choice between a rename or genuine
`navigator.clipboard` JavaScript "if it can be done safely without
adding fragile frontend code," and the client's own explicit acceptance
that "Show Agent Key is acceptable as long as the full key can be
selected and copied manually" (already true, unchanged), the action is
renamed `action_copy_agent_key()` → `action_show_agent_key()`, with the
button label updated to "Show Agent Key" - model, view, and all 5
existing tests updated together. The underlying mechanism is completely
unchanged: the same `sticky: True` `display_notification`, the same
`groups="flexsys_kds.group_kds_administrator"` restriction with
server-side `AccessError` defense in depth, the same guarantee that
`agent_key` is never written to or regenerated by this action. 403
automated tests (unchanged count - existing tests renamed, not added
to), all `py_compile`/XML/JS checks passing, plus a custom AST-based
