...
Odoo instance connected to the client's own database - every round of
this investigation so far has been static code review, which has
already been exhausted (`_station_from_token()` re-read character by
character multiple times across two prior rounds, every one of the
four public kiosk routes traced, the exact `'printer_only'` string
verified against the field's own Selection definition). The client's
own explicit, correct request is for runtime proof against their real
deployment - something only their own environment can actually
produce. This version does not claim to have found or fixed the root
cause; it adds the diagnostic instrumentation needed to get that proof
in one reproduction, and reports every remaining hypothesis considered
and why each does or doesn't fit the reported symptom.

### What was added
`_station_from_token()` - the single, central function every one of
the four public kiosk routes calls - now logs, at INFO level (visible
in a standard server log with no configuration change needed), the
answer to each of the five questions the client asked directly:
which station resolves for a given `(station_code, token)` pair, that
station's own real `id`/`name`/`company_id`/`operating_mode`/
`kiosk_disabled` values at the moment of the request, and an explicit
ALLOWED or REJECTED-with-reason outcome for every decision point in
the function (missing input, no matching station, token mismatch,
`kiosk_disabled`, `operating_mode == 'printer_only'`, or success).
Search the server log for `FLEXSYS_KIOSK_AUTH` after reproducing the
report once - the log lines answer questions 1-5 directly, from the
real request, not a re-reading of the source.

### Hypotheses considered this round, each evaluated against the reported symptom
1. **`code` is unique only per `(code, company_id)`, not globally**
   (`_code_company_uniq`, `kds_station.py`) - in a genuine multi-
   company setup, two DIFFERENT stations could share the same `code`
   in different companies, and the database search (unfiltered by
   `company_id`) could resolve the wrong one. **Does not fit the
   reported symptom on its own**: `kiosk_token` is a per-station random
   value unrelated to `code` - if the wrong station were resolved, the
   token comparison immediately after would almost certainly fail
   (rejecting the request outright), not silently succeed. Still worth
   ruling out via the new logging's own `company_id`/`company_name`
   fields, in case the client's own environment has a company
   configuration this reasoning didn't anticipate.
2. **Browser/proxy caching** (v7.27.1's own hypothesis) - addressed
   with explicit `no-store`/`no-cache` response headers on the initial
   page. Not re-litigated here; the new logging will also settle this
   directly - if `FLEXSYS_KIOSK_AUTH: REJECTED... printer_only` appears
   in the log for the exact request the browser shows as successful,
   this was never a server-side rejection reaching the browser at all,
   which points squarely at caching (or a CDN/reverse proxy in front
   of Odoo the response headers alone might not reach) rather than the
   auth logic.
3. **The running Odoo service never actually loaded this code** -
   Odoo loads Python controller code into memory once, at worker
   startup; a code change on disk has no effect until the service is
   genuinely restarted (a module "Upgrade" from the Apps UI updates
   data/views but does not by itself guarantee a Python-level reload
   depending on the deployment's own process model). If NO
   `FLEXSYS_KIOSK_AUTH` line appears in the log at all after
   reproducing the report, this is the confirmed explanation - the
   deployed code is not the code being reviewed here.

### What this version deliberately does not do
Does not change the actual authorization decision in any way - the
existing `operating_mode == 'printer_only'` check, already confirmed
correct on every static re-read, is untouched; only logging was added
around it. No workaround, no token change, no caching change beyond
what v7.27.1 already added. Per the client's own explicit instruction:
root cause first, fix only once confirmed.

### Files changed
`controllers/kds_kiosk.py` (`_station_from_token()`'s own diagnostic
logging only).

### Tests
1 new test confirming the logging genuinely covers every decision
point in the function, and functionally confirms (via Python's own
`assertLogs`) that calling the function against a `printer_only`
station actually produces the expected `FLEXSYS_KIOSK_AUTH... REJECTED
... printer_only` log line - the closest confirmation available
without a live deployment to reproduce the original report against
directly.

**Total: 540 tests** (up from 539). No database migration required -
one function instrumented with logging only.

**Required before any further fix is attempted**: reproduce the exact
reported scenario once against this version, then search the server
log for `FLEXSYS_KIOSK_AUTH` and share what it shows - this determines
which of the three hypotheses above (or a fourth this round didn't
anticipate) is the actual root cause, and what the correct fix
actually is.

---

## v7.27.1 — Final Cleanup Bug: Printer Only Kiosk Still Accessible — Investigation Report + Defense-in-Depth

**Important, stated plainly rather than glossed over**: a thorough,
line-by-line re-verification of `_station_from_token()` and every one
of the four public kiosk routes that call it found the backend
`operating_mode == 'printer_only'` check genuinely present,
unconditional, and correctly wired into all four - no code defect was
located this round. This delivery adds a real, server-side defense-in-
depth measure (HTTP caching headers, described below) plus the six
required regression scenarios, and reports the investigation findings
in full for the client's own next live re-test.

### Investigation performed
Traced the complete request path from the URL to the data, exactly as
requested: confirmed via a full codebase search that this module
defines exactly four `@http.route`s total, all four inside
`controllers/kds_kiosk.py`, and all four - the initial page
(`kiosk_page`), and the three API endpoints (`kiosk_orders`/
`kiosk_action`/`kiosk_print`) - call `_station_from_token()` and
correctly short-circuit with a rejection when it returns nothing; no
alternate route, controller, or module anywhere in this codebase
resolves a station without going through this exact function.
Re-read `_station_from_token()` itself character by character: the
`operating_mode == 'printer_only'` check added in v7.27.0 is genuinely
present, unconditional, and returns `None` immediately, before the
station is ever returned to any caller. Directly verified the exact
string `'printer_only'` matches the field's own Selection definition
(`kds_station.py`) precisely - no typo, no value mismatch.

### Most likely explanation, given the backend logic checks out correctly
Since the server-side logic itself is confirmed correct on every static
re-read, the most likely explanation for a request that still succeeds
against a station now correctly rejected server-side is the BROWSER
(or an intermediate cache/proxy) serving a stored copy of the kiosk
page from an earlier, still-valid visit, without ever re-issuing the
request to this server at all. The kiosk page's own HTTP response
previously carried no caching directives at all, leaving browsers free
to decide for themselves whether to cache it.

### Fix - defense-in-depth, server-side, per the client's own explicit requirement
`kiosk_page`'s own HTTP response now explicitly sends
`Cache-Control: no-store, no-cache, must-revalidate, max-age=0`,
`Pragma: no-cache`, and `Expires: 0` - standard HTTP response headers
sent directly by the server, instructing every browser and
intermediate cache/proxy that no stored copy of this response may ever
be served under any circumstance; every single request for this URL
must reach the real handler, every time, so a station's own CURRENT
operating_mode is always what gets checked. This is genuinely
server-side enforcement via a standard protocol mechanism - not a
JavaScript redirect, not client-side state, not anything the browser
could choose to ignore. The three JSON-RPC API endpoints are typically
requested via POST, which browsers do not cache by default, so this
was not extended to them.

### If the issue still reproduces after this fix
This would indicate the browser/proxy caching explanation was
incomplete or incorrect, and points toward a deployment-layer question
instead: whether the Odoo service running the live environment was
actually restarted/the module actually upgraded after the v7.27.0 code
was deployed - Odoo loads Python controller code into memory once, at
worker startup, and does not pick up changes to `.py` files on disk
until that happens. Confirming this (or ruling it out) is the most
useful next diagnostic step if the fix in this version does not
resolve the report.

### Required regression tests, all added
Six scenarios, verbatim from the report, all exercising the real,
unmodified `_station_from_token()` directly: KDS Only + valid token
(allowed); KDS + Printer + valid token (allowed); Printer Only + valid
token (rejected); a station changed from KDS Only to Printer Only
while retaining its own original token (old URL rejected - the exact
reported reproduction); Printer Only reverted back to KDS Only (access
restored with the same token, no regeneration); plus a structural
confirmation that all three API endpoints genuinely call the shared
gate (not merely the initial page). One additional test confirms the
new cache-prevention headers are present in the real source.

### Explicitly untouched, per the request's own scope control
Quantity reconciliation, POS delta logic, READY/COMPLETED lifecycle,
Routing, Printing logic, Cancellation/refund behavior, and the already-
passed Priority/Urgent/VIP UI cleanup (v7.27.0) - no file related to
any of these was modified in this round.

### Files changed
`controllers/kds_kiosk.py` (`kiosk_page()`'s own response headers
only - the auth logic itself, already confirmed correct, was not
modified).

### Tests
7 new tests: the six required scenarios verbatim, plus the new cache-
header confirmation.

**Total: 539 tests** (up from 532). No database migration required -
one HTTP response header addition, no field/model changes.

**Required before this can be closed**: the client's own live re-test
of the exact same previously-saved Public Kiosk URL while the station
remains in Printer Only, on this specific version, with the deployment
question above investigated if the issue still reproduces.
...
other work.** `19.0.7.13.0` was confirmed to prevent the POS register
from opening at all - `TypeError: Cannot read properties of undefined
(reading 'currency_id')` inside Odoo's own
`PosStore.processServerData()`, occurring during POS initialization,
before any Offline Send testing could even begin.

### Immediate action taken
Both v7.13.0 frontend pieces are reverted completely: the
`_load_pos_data_fields()` override on `pos.order` (the confirmed
primary suspect) and the paired JS increment patch
(`static/src/js/flexsys_kds_send_generation.js`), along with the
`point_of_sale._assets_pos` manifest bundle entry that loaded it. This
module is once again entirely backend-only, touching no POS register
frontend code at all - the same confirmed-safe state as v7.12.1.

### Root cause - honestly not yet fully confirmed
Two sources consulted while designing v7.13.0 directly conflicted:
one presented `_load_pos_data_fields()` on any POS-loaded model,
including `pos.order` directly, as the standard, safe, documented Odoo
19 mechanism; a separate, independent source explicitly warned "your
approach using `_load_pos_data_fields()` is not correct for POS
orders... the POS frontend crashes" for the closely related Odoo 18.
The live crash now confirms the second source was correct for this
exact case - but the deeper reason why (whether `pos.order`'s own
native field-loading path has additional, undocumented constraints
this override violated, an ordering dependency, a naming collision, or
something else this delivery has not yet isolated) has not been
independently re-verified against Odoo 19's own actual runtime. This
is stated plainly rather than guessed at a third time.

### What is explicitly unchanged - the backend architecture
Per the client's own explicit instruction ("Do NOT remove the durable
generation architecture... The immediate task is ONLY to fix how
`kds_send_generation` is exposed"): `kds_send_generation` and
`kds_last_processed_send_generation` (the database fields themselves),
`_flexsys_kds_process_one_sync_from_ui_entry()`'s own generation-based
authorization and idempotency logic, and every other part of v7.12.1's
own architecture are completely untouched and remain fully functional
- confirmed by new, direct tests that construct the `sync_from_ui`
payload manually, with no dependency on any frontend field-loading
mechanism at all.

### Currently open again
The one remaining gap - a verified, safe way to expose
`kds_send_generation` to the POS frontend and increment it on a
genuine Send - needs a different, independently-verified approach
before being reattempted. Not guessed a third time between the two
conflicting sources this round's own failed attempt was based on.

### Files changed
`models/pos_order.py` (the breaking override removed;
`kds_send_generation`'s own field docstring updated with the honest,
current status), removed `static/src/js/flexsys_kds_send_generation.js`,
`__manifest__.py` (`point_of_sale._assets_pos` bundle entry removed).

### Tests
4 tests replacing the 4 removed obsolete ones (which tested the
now-reverted override directly): confirm both generation fields
themselves remain fully intact at the database level; a guard-rail
test confirming `flexsys_kds` does not currently contribute its own
`_load_pos_data_fields()` anywhere in `pos.order`'s own MRO (protects
against silently reintroducing this exact crash without independent
live verification); and confirming the backend's own generation-based
authorization logic remains fully functional using a manually-
constructed payload, independent of any frontend mechanism.

**Total: 353 tests** (net unchanged - 4 removed, 4 added). No database
migration required - no field or schema changes this round.

**This is the single highest-priority item for the next live test
cycle**: confirm POS opens normally on a real Odoo 19 instance before
any further Offline Send Recovery work is attempted.

---

## v7.13.0 — Frontend Durable Send Generation: the missing increment, added narrowly and defensively
...
and not to be redesigned) is now paired with the one remaining gap: an
actual increment of `kds_send_generation` at the moment of a genuine
cashier Send.

### 1. Exact hook
`PosStore.prototype.sendOrderInPreparation(order, opts)` -
`static/src/js/flexsys_kds_send_generation.js`. Confirmed directly from
Odoo 19's own core source
(`addons/point_of_sale/static/src/app/services/pos_store.js`) to be the
method the native "Send" action calls, and confirmed by the client's
own live Network A/B test to be the exact boundary between an ordinary
edit (zero related calls) and a genuine Send
(`get_preparation_change` -> `sync_from_ui`).

### 2. Why this point represents a genuine Send
It is the same method the client's own confirmed evidence already
identifies as Send-exclusive (the A/B test that led to v7.11.0's own
`get_preparation_change()`-based design). This patch does not
introduce a new signal or a new theory about the call chain - it
increments a field on the exact object (`order`, the method's own first
parameter - Odoo's own local, offline-first POS order model) already
proven to be present at the moment of a genuine Send.

### 3. Duplicate-increment prevention
Not prevented at the frontend layer - deliberately. The backend's own
authorization comparison (`incoming_generation > kds_last_processed_send_generation`,
strictly-greater-than, not exact equality) already tolerates a
multi-step advance without any risk of duplicate processing (v7.12.1's
own design, unchanged). If this hook is ever invoked more than once for
a single logical cashier Send (not confirmed either way without direct
live access), the practical result is, at most, a harmless "skipped"
generation number - never a duplicate `kds.order`, line, or event, since
the backend's own idempotency key (`uuid` + `generation`) already
guarantees each distinct generation value is processed exactly once
regardless of how many times it happens to arrive.

### 4. Persisted while offline
The increment is a synchronous, in-memory mutation of the local `order`
object's own field - no RPC call of its own at all, unlike both
earlier, now-removed frontend patches (v7.9.3, v7.9.6), which is
precisely why those failed for the offline case this fix targets.
Because `kds_send_generation` is loaded into the POS session via
`_load_pos_data_fields()` (see item 6), Odoo's own offline-first POS
architecture already handles persisting and restoring this value across
a disconnect/reconnect or a local browser reload automatically -
exactly the same way it already does for every other native
`pos.order` field, `last_order_preparation_change` included.

### 5. How it enters sync_from_ui after reconnect
Automatically, as part of the SAME order-save payload Odoo's own
offline queue already reliably retries - no additional code needed;
this is the entire point of choosing a plain order field over a
separate RPC call.

### 6. Ordinary edits confirmed never incrementing it
The increment lives exclusively inside this one patched method. No
other code path in this delivery touches `kds_send_generation` on the
frontend side at all - an ordinary edit (quantity change, line
add/remove, note, modifier) never calls `sendOrderInPreparation()`
(confirmed by the client's own live A/B test: zero `get_preparation_change`
calls for an ordinary edit), so it can never reach this increment
either.

### 7. Server-owned fields confirmed protected
`kds_last_processed_send_generation` is deliberately NOT added to
`_load_pos_data_fields()` - it is never loaded into the POS frontend's
own local order model at all, so there is no local copy for the client
to read, cache, or write back, in addition to the existing
`_KDS_SERVER_OWNED_FIELDS` sanitization (v7.11.3, unchanged) that would
strip it from an incoming payload regardless. `kds_send_generation`
itself is the one, intentional exception - loaded specifically because
the POS client is meant to write it.

### New: `_load_pos_data_fields()` override
Confirmed as Odoo 19's own documented mechanism (`pos.load.mixin`) for
exposing a field to the POS frontend's own local order model without
any further frontend JS change - `pos.order` already inherits this
mixin as a core POS model; this override simply appends
`kds_send_generation` to the list of its own fields the POS session
loads at startup, and Odoo's own architecture handles the rest
(loading, offline persistence, serialization) automatically.

### Files changed
`models/pos_order.py` (`_load_pos_data_fields()` override; both new
fields' own docstrings updated to reflect the completed design), new
`static/src/js/flexsys_kds_send_generation.js`, `__manifest__.py`
(`point_of_sale._assets_pos` bundle reintroduced, deliberately minimal
- exactly one file).

### Tests
4 new tests confirming `_load_pos_data_fields()`'s own exact
contract: `kds_send_generation` is included; `kds_last_processed_send_generation`
and the other two purely-internal fields are never included; the
native field list itself is preserved, not replaced. The frontend
increment logic itself cannot be tested by this delivery process
without a live Odoo 19 browser session - stated plainly, not glossed
over; every part of the backend comparison and idempotency logic it
feeds into was already covered by v7.12.1's own test suite.

**Total: 353 tests** (up from 349). No database migration required.

**Per the dev report's own explicit final instruction**: this delivery
does not claim Offline Send Recovery is complete based on these unit
tests alone. Final acceptance requires the client's own live test:
disconnect POS internet -> press Send -> reconnect -> `kds.order` must
appear automatically, exactly once.

---

## v7.12.1 — Removed unsafe offline fallback; added durable kds_send_generation; fixed consume-before-delivery

**Critical correction, from the client's own architectural review of
v7.12.0, before it was ever deployed to a live instance.** Two
separate, serious issues, both fixed here.

### Issue 1 - the v7.12.0 content-signature fallback was architecturally unsound
The client's own review proved it directly: "content changed != cashier
pressed Send." An ordinary, unsent quantity edit and a genuine Send
performed while offline both eventually reach the server through
`sync_from_ui` with content that differs from the last committed KDS
snapshot - nothing about the content alone can distinguish the two.
Continuing to compare `last_order_preparation_change`'s own content in
any form reintroduces the exact "ordinary edit leaks to KDS" bug this
project spent several earlier rounds (v7.9.1 through v7.11.3)
confirming and fixing. **The v7.12.0 fallback is removed entirely, not
adjusted** - the client's own explicit instruction ("do NOT release
another backend signature fallback") is honored precisely: this round
does not patch the removed mechanism, it replaces the underlying
strategy.

### Issue 2 - authorization was consumed before delivery succeeded
`kds_preparation_change_requested` used to be cleared (and
`kds_last_processed_send_signal` updated) BEFORE `_flexsys_kds_sync()`
was called - if that call then failed, the Send would already be
marked processed, permanently lost with no possibility of retry. Fixed
by reordering: `_flexsys_kds_sync()` now runs first; the authorization
marker(s) are only cleared/advanced AFTER it completes without raising.
A failed attempt leaves the marker(s) untouched, so a subsequent retry
(Odoo's own or an ordinary re-save) correctly re-attempts it.

### The correct fix for offline-Send recovery: kds_send_generation
Per the client's own recommended architecture: a durable counter,
exclusively under this module's own control, that an ordinary edit
never touches - only a genuine Send would increment it. Being a plain
field on `pos.order` itself, it rides along on the SAME reliably-
retried `sync_from_ui` payload the order's own business data already
uses (confirmed live to survive offline recovery), rather than
depending on a separate RPC call (`get_preparation_change()`) that
Odoo's own offline-first architecture doesn't guarantee retrying.
`kds_last_processed_send_generation` (the server's own bookkeeping of
what's already been processed) is added alongside it, protected by the
existing sanitization mechanism (`_KDS_SERVER_OWNED_FIELDS`) -
`kds_send_generation` itself is deliberately NOT protected, since it's
specifically meant to be written by the POS client.

**Honest, explicit status - the backend half is correct, the frontend
half does not exist yet.** No frontend mechanism currently increments
`kds_send_generation` on Send - the exact missing piece the client's
own questions 1-4 concern. Shipping the backend comparison logic alone
is safe regardless: with nothing yet writing a genuinely incrementing
value, incoming and last-processed generation both stay at their
shared default (0) and this path authorizes nothing on its own - ready
to be exercised the moment a verified frontend increment exists,
without a further backend change at that point. Adding that frontend
piece without first verifying the correct hook point live would repeat
this project's own hard-learned lesson from the two frontend patches
(v7.9.3, v7.9.6) added without that verification and later removed
entirely in v7.11.0 - not repeated here.

### Files changed
`models/pos_order.py` (`kds_send_generation`,
`kds_last_processed_send_generation` fields;
`_KDS_SERVER_OWNED_FIELDS` updated;
`_flexsys_kds_process_one_sync_from_ui_entry()`'s own authorization and
ordering rewritten).

### Tests
10 new tests, replacing the 5 removed obsolete ones: re-confirms the
core invariant directly against the exact scenario the removed
fallback would have violated; the consume-after-success fix (a failed
sync leaves the marker untouched; a successful one still consumes
correctly; a failed-then-retried Send succeeds exactly once on retry);
and the full `kds_send_generation` comparison logic (inert at default
state; a genuine increment authorizes; repeated retries of the same
generation are idempotent; a second increment applies the next delta;
a stale/lower generation never authorizes or regresses; the field
itself correctly survives payload sanitization while the purely
server-owned generation-tracking field is still stripped).

**Total: 349 tests** (up from 344, net of 5 removed and 10 added). No
database migration required - both new fields default to 0, correct
for every existing record.

---

## v7.12.0 — Offline POS Send recovery: content-signature fallback authorization

**Confirmed live**: create order → disconnect internet → press Send
while offline → stay offline ~2 minutes → reconnect without refreshing.
The POS order's own business data reliably recovers, but no
`kds.order` is ever created - the explicit Send-to-KDS intent is lost
even though the cashier believes it was sent.

### Root cause
`get_preparation_change()` is, from Odoo's own core perspective, a
side-effect-free call that fetches data for the print/preparation flow
- not a durable write Odoo's own offline-first POS architecture treats
as needing guaranteed retry, unlike the order's own persisted data
(delivered through `sync_from_ui`'s own payload), which IS reliably
queued locally and retried on reconnect (confirmed live: "the POS
order itself eventually synchronizes back to the server"). If the
device is offline at the exact moment `get_preparation_change()` is
called, that specific RPC simply never reaches the server and, unlike
the order's own save, is never retried - so
`kds_preparation_change_requested` never gets set, even though the
cashier's own intent to Send was genuine. The failure occurs before any
KDS ingestion logic runs at all, exactly as the dev report's own
diagram describes.

### Fix - a fallback riding on Odoo's own confirmed offline guarantee
If `kds_preparation_change_requested` is not set, but a `sync_from_ui`
call's own content signature genuinely differs from
`kds_last_processed_send_signal` (the last content actually processed
as an authorized Send), this is now ALSO treated as authorization -
using the one piece of data confirmed to survive an offline period
reliably: the order's own persisted `last_order_preparation_change`
content, delivered through the same `sync_from_ui` retry mechanism
already proven to recover the order's own business data. No new
frontend queue is needed - the retried `sync_from_ui` call itself IS
the retry.

Idempotent by the same construction already in place: the fallback
updates `kds_last_processed_send_signal` upon triggering, so repeated
retries of the same content are correctly skipped on subsequent
attempts - satisfying the required exactly-once guarantee without a
separate idempotency key, since the content signature itself already
serves that role.

### Honest, explicitly-documented limitations
- This is a **backend-only** fix. It directly closes the confirmed
  symptom (missing `kds.order`), satisfying the dev report's own core
  requirement ("the fix belongs in POS Send delivery / retry /
  acknowledgement logic," not the KDS display layer) - but does **not**
  implement the dev report's own Option B (a durable, explicit
  "Pending Kitchen Send / Not Sent" UI indicator on the POS register
  itself) or a full local retry queue (Option A) - both would require
  frontend JS work this project's own history has repeatedly shown to
  be high-risk and difficult to verify without direct live access (see
  the now-entirely-removed v7.9.3/v7.9.6 frontend patches).
- This fallback's own correctness rests on the assumption that an
  *ordinary* edit's content does not coincidentally trigger it - not
  independently re-verified under the current, corrected signature-
  management code (v7.10.2/v7.11.3's own fixes). Logged at **WARNING**
  level specifically (contrasting with this method's otherwise
  INFO-level logging) so this fallback's own real production frequency
  is directly observable - if it fires unexpectedly often outside
  genuine offline-recovery scenarios, that's itself actionable evidence
  for a live investigation, not something assumed away here.

### Files changed
`models/pos_order.py`
(`_flexsys_kds_process_one_sync_from_ui_entry()`'s own authorization
logic extended with the fallback path).

### Tests
5 new tests: the dev report's own Required Acceptance Tests 1-3
(offline initial Send recovers without the flag; repeated retries
produce no duplicates; an offline second Send/delta after an online
first Send applies exactly once), confirming the fallback correctly
does nothing when content is genuinely unchanged (not a blanket
"always sync" fallback), and confirming the fallback path logs at
WARNING level with a recognizable marker for production monitoring.

**Total: 344 tests** (up from 339). No database migration required.
...
caught by inspection before it shipped further): two issues in v7.7.0's
own "On Send to KDS" implementation.

### 1. `write()`'s own gate could skip the Send signal entirely
`if 'state' in vals or 'lines' in vals:` was the entry condition for
the whole KDS sync block - but the native Send signal
(`last_order_preparation_change`) can arrive in a `write()` call on its
own, with neither `state` nor `lines` also present in that same vals
dict. That write never entered the block at all, so `is_send_write` was
never even computed and `_flexsys_kds_sync()` was never called - "On
Send to KDS" mode could silently never sync anything, since the one
write meant to trigger it didn't reach the trigger check in the first
place. Fixed by adding `'last_order_preparation_change' in vals` to the
gate condition itself, not just using it inside the block. An existing
test (`test_native_send_signal_triggers_initial_sync`, from v7.7.0)
already exercised this exact scenario correctly - it had simply never
been executed against a real Odoo test runner yet, only `py_compile`-
checked; this fix should now make it pass for real.

### 2. `unlink()` redesigned so early-leak is structurally impossible, not just assumed safe
v7.7.0 canceled a removed line immediately in `pos_order_line.py`'s own
`unlink()`, resting on an **unverified assumption** that a backend
unlink() call only ever arrives as part of the same batched request
that also carries the Send/New signal - explicitly flagged as needing
live verification at the time. Rather than leave that assumption
unverified, redesigned so correctness no longer depends on it at all:

- New `pending_removal` field (`kds_order_line.py`) - `unlink()` now
  only sets this flag (a plain write touching neither `state` nor
  anything a KDS screen renders), preserving the fact that removal
  happened without making it visible yet.
- `_flexsys_kds_diff_lines()` (`pos_order.py`) processes any
  `pending_removal` lines right at its own start - the one place that
  only ever runs at the correct sync boundary for whichever trigger
  mode is configured, since it's never reached except through
  `_flexsys_kds_sync()`'s own gate. Applies the same state-aware
  cancellation as before (Completed lines via
  `_system_cancel_after_completion()`, active lines via the real
  `action_cancel()`), just deferred to the genuine sync point instead
  of unlink() time.
- **A regression caught while implementing this**: the new
  `pending_removal`-setting query in `unlink()` initially excluded
  `'completed'` lines (matching the *old* code's own search filter) -
  which would have silently lost the "Change Request After BUG-11" item
  1 capability entirely (a POS line deleted after its KDS line already
  reached Completed would never even get flagged). Caught and fixed
  before it shipped - only `'cancelled'` is excluded now;
  `_flexsys_kds_diff_lines()` itself decides how to handle each
  remaining state.

**4 existing tests individually updated** to match the new two-step
contract (unlink() alone changes nothing visible; the *next* genuine
Send/New signal applies the real cancellation) - each now explicitly
confirms the line is unchanged immediately after `unlink()`, then
triggers a second Send signal and confirms the cancellation applies at
that point. This removes the "verify against real Odoo 19 POS
behavior" limitation entirely for this specific mechanism - it is now
correct by construction regardless of how POS's own frontend batches
its requests, not because of an assumption about it.

### 3. Manifest/README description corrected
Still described "on payment, order validation, or on POS submit" -
updated to reflect the actual two supported options (After Payment / On
Send to KDS) in both `__manifest__.py` and `README.md`.

No test count change (248) - all fixes this round corrected existing
mechanisms/tests rather than adding new scenarios to cover.

No database migration required beyond the new `pending_removal` field
(auto-added on upgrade, defaults `False`).

---

## v7.7.0 — Change Request After BUG-11: 3 items (deleted-completed-line cancellation, negative delta, Send-to-KDS trigger simplification)

Implements the formal "Change Request After BUG-11" - three items
identified during live functional testing.

### Item 1: Completed order - deleted POS line must become CANCELLED
A cashier deleting an already-completed product from a COMPLETED POS
order left the KDS line displayed as if still normally completed - the
existing line-removal detection explicitly excluded `'completed'`
lines from ever being cancelled. New
`kds_order_line.py::_system_cancel_after_completion()` - a dedicated
internal path (never exposed via any UI action - the normal
`action_cancel()`'s own "cannot cancel a Completed line" restriction
stays fully intact for every real user-facing path) mirroring
`action_cancel()`'s own audit/notification behavior exactly. The order
itself is never touched - `is_expeditor_ready` already excludes
cancelled lines, so cancelling one Completed line while its Completed
siblings remain Completed still correctly reads as "everything
remaining is done." 2 new tests.

### Item 2: Quantity decrease delta - a real backend gap, not just display
Investigation found the true root cause went deeper than display: a
quantity *decrease* on an already-Ready line was previously ignored
entirely (logged only) - the displayed quantity never actually reduced
and no delta showed at all, regardless of sign. Fixed by splitting
Ready from Completed: **Completed** keeps the original informational-
only, never-mutate-history behavior; **Ready** now reduces the quantity
in place - "do not reopen unnecessary production" (no delta line, no
state reset) - while correctly showing `UPDATED (-N)` via the existing
`qty_delta` mechanism. 5 new tests (including confirming the
already-working active-line case is unaffected, and that Completed's
own informational-only behavior is unchanged).

### Item 3: POS Send-to-KDS trigger simplification
Simplified `kds_send_trigger` from three options to exactly two -
`payment` ("After Payment", unchanged default behavior) and `send`
("On Send to KDS", replacing both the old `validation` and `submit`
options, which are now removed).

**Root cause of the old options' own violation**: both previously
triggered sync on *any* backend write touching `lines`/`state` at
all - creating, editing, adding, removing, simply re-saving - directly
violating "Critical Trigger Rule: adding/removing products, changing
quantities... must NOT automatically synchronize... The synchronization
boundary must be the cashier's explicit action: Send or New."

**Fix, using Odoo's own native workflow (not a new custom button)**:
confirmed directly from Odoo 19's own core source
(`addons/point_of_sale/models/pos_order.py`) that `pos.order` has a
`last_order_preparation_change` field ("Last printed state of the
order"), updated by the native Preparation-Display "Send" action.
`'send'` mode now requires this field to be present in the same
`write()`/`create()` call before syncing - every other edit correctly
accumulates with zero KDS sync until the next genuine Send.

**A second, real gap found and fixed while implementing this**:
`pos_order_line.py`'s own create/write hooks called
`_flexsys_kds_diff_lines()` **directly**, completely bypassing the new
trigger gate - meaning even under `'send'` mode, editing an *existing*
line (qty change, note change) still synced immediately regardless.
Routed through `_flexsys_kds_sync()` instead, so both hooks now
correctly respect whichever trigger mode is configured.

**Honest, explicit limitation** (documented here and in
`RELEASE_STATUS.md`, not glossed over): the "Send" signal
(`last_order_preparation_change`) is confirmed from Odoo's own source
for Scenario 1 (Preparation Display enabled). Whether the native "New"
action (Scenario 2, Preparation Display disabled) also updates this
same field has not been confirmed against a live instance - genuinely
needs live verification. Deliberately fails *closed* under this
uncertainty (nothing reaches the kitchen under an unconfirmed
configuration) rather than *open* (syncing too early), the safer
direction for this specific unknown.

**9 existing tests individually reviewed and updated** (not batch-
replaced) - 2 were removed entirely since they tested the old,
now-incorrect "sync on any draft order" behavior directly; the other 7
were updated to use `'send'` plus the new required Send signal, each
reviewed for whether the signal actually needed to be present for that
specific test's own assertions to remain meaningful (one test's own
"no lines yet" assertion held either way; the rest needed it added).
7 new tests covering the required scenarios: default trigger, exactly
two options remain, no sync without Send, sync on native Send,
accumulated changes syncing correctly together as ADDED/UPDATED/
CANCELLED, and removal-before-Send showing CANCELLED once synced.

### Files changed
`models/kds_order_line.py` (`_system_cancel_after_completion()`),
`models/pos_order.py` (line-removal wiring, quantity-decrease fix,
trigger signature/logic), `models/pos_order_line.py` (hooks routed
through the authoritative gate), `models/pos_config.py` (simplified
Selection), `tests/test_pos_sync.py` (13 new tests, 9 existing tests
individually reviewed and updated, 2 obsolete tests removed).

**Total: 248 tests** (up from 237). No database migration required.

---

## v7.6.0 — BUG-11: paid order refund now reconciles back to the original KDS ticket

**Confirmed live on Odoo.sh** (order 262-4-000013, 2 x Lunch Temaki mix
3pc, PREPARING): a partial refund (1 of 2) left KDS completely
unchanged. A second refund of the remaining 1 (net 0, fully refunded)
still showed "2 x ... PREPARING" with READY still available -
"operationally dangerous."

### Root cause
BUG-06 correctly stopped a refund order from creating its **own** new
KDS ticket - but stopped there. The original ticket was never told
anything had changed at all, so it kept showing the full, un-refunded
quantity indefinitely regardless of how much was actually refunded.

### Fix
New `pos_order.py::_flexsys_kds_reconcile_refund()`, called instead of
silently returning once a refund order is detected - correlates each
refund line back to its original `pos.order.line` via
`refunded_orderline_id` (checked for existence first, same defensive
posture as the existing refund-detection method), then to the
**original** `kds.order.line` via `pos_order_line_id`. Implements the
required concept exactly: `effective_kitchen_qty = original_qty -
cumulative_refunded_qty`, recomputed fresh from every refund line
against that original every time (not a running counter), making
reconciliation cumulative and idempotent by construction - reprocessing
the same or overlapping refunds converges on the same correct answer
rather than double-subtracting.

**Never creates a new KDS ticket or a negative production line** - this
method only ever reduces or cancels an existing line, the same
authoritative-backend principle BUG-06 already established.

**Stage-specific behavior, exactly as specified:**
- **Fully refunded** (New/Preparing/Ready): cancelled through the real
  `action_cancel()` - full audit trail, the same CANCELLED display and
  retention every other cancellation already has.
- **Partially refunded** (New/Preparing/Ready): quantity reduced in
  place, current *state* never touched - "do not reopen unnecessary
  production" for a Ready line; a Preparing line stays Preparing.
  Reduction shows as `UPDATED (-1)` via the same `qty_delta` mechanism
  BUG-09 already established.
- **Already Completed** (partial or full): the original line is never
  mutated - "the original completed work remains historically
  completed" (same principle as BUG-02B/BUG-10) - recorded as an
  informational audit event only, never new production work.

### Tests: 12 new, matching the dev report's own required list 1:1
Preparing/Ready partial and full refunds, cumulative full refund across
two separate refund orders (the exact live scenario), Completed partial
and full refunds (informational only), no new ticket, no negative
quantity (including a defensive over-refund check), cumulative
reconciliation across three separate refund orders, idempotent
re-processing, multi-station isolation (refunding one product never
touches a different product on a different station), and audit history
preserving both the original production lifecycle and the refund's own
events separately. New `_create_refund_order()` test helper - skips
gracefully (not a failure) if `refunded_orderline_id` isn't present on
a given build, since there is no reliable way to test the correlation-
dependent reconciliation without it.

**Confirmed no regression**: all 4 existing BUG-06 refund tests remain
logically correct unchanged - they never set `refunded_orderline_id` on
their own synthetic refund lines, so the new reconciliation logic
correctly finds nothing to correlate against and performs no action,
exactly matching what those tests already expected.

**Total: 237 tests** (up from 225).

No database migration required. No new fields - reuses `qty_delta`
(BUG-09) and the existing cancellation/audit infrastructure throughout.

---

## v7.5.0 — BUG-10 (mislabeled): READY line incorrectly reset to NEW on POS quantity increase

**Confirmed live on Odoo.sh**: a Ready order whose quantity was
increased from POS reset the entire line back to NEW, with an active
START button, "as if the complete order had started a fresh production
cycle" - even though the delta itself was correctly detected and
displayed as `UPDATED (+1)`.

### Root cause
`pos_order.py::_flexsys_kds_diff_lines()` had `if kline.state ==
'ready': kline._system_reset_for_delta_sync('new')` - a Ready line
whose qty/note/variant changed was bumped fully back to `'new'`,
destroying the fact the previous quantity had already been prepared.
This was a deliberate design choice from an earlier round (BUG-02), but
this dev report makes explicit what the correct behavior actually needs
to be: the previously-prepared quantity must stay preserved as Ready
production history; only the genuine increase needs a new production
cycle.

### Fix
Unified into the exact same "create a new delta line, never mutate the
original" pattern already established for a Completed line's own
equivalent case (built for BUG-02B) - both `'ready'` and `'completed'`
now share one branch. The original line is never mutated - keeps its
own state, qty, `ready_time`, exactly as before. A new delta line is
created instead, carrying the same `pos_order_line_id` so future diffs
correctly match against it.

**Found and fixed a second, related bug while checking the already-
existing Completed-line branch for the same underlying issue** (per
explicit instruction to verify old passing tests are still logically
correct, not just still passing): that branch's own delta line used to
carry `qty = line.qty` (the full new POS total, e.g. 7 for a 5->7
change) instead of just the increase (2) - silently double-counting the
already-completed units. Fixed for both states at once: the delta
line's qty is now the genuine increase only, except for a note/variant-
only change (qty unchanged), where the delta line correctly carries the
full current quantity instead - there is no sensible "partial delta"
for reconfiguring an already-prepared batch.

**Found and fixed a third bug this exposed**: a *second* consecutive
modification (5 completed -> 7 via a first delta of 2 -> 10) used to
overwrite the delta line's own qty with the new POS total directly (10),
again double-counting the original's own 5. The delta line's own qty is
now correctly computed as the POS total minus any historical (Ready/
Completed) sibling line for the same POS line, so a second modification
correctly updates the existing delta rather than creating a duplicate
or double-counting.

Decreases on an already Ready/Completed line create no delta line at
all (there is no new production work to represent, and a negative-qty
production line makes no operational sense) - just an audit note,
leaving the existing production history untouched, matching the
already-established pattern for a completed line reduced to zero.

`_system_reset_for_delta_sync()`'s one remaining call site was removed
(its Ready-line-reset behavior was exactly the bug); the method itself
is kept defined, documented as currently unused, not deleted.

### Tests
Audited every test file for any dependency on the old reset-to-new
behavior (only `test_pos_sync.py` had any) and updated 4 existing tests
whose assertions checked the now-corrected behavior, rather than
leaving them checking the old, wrong quantities. One new comprehensive
end-to-end test covering the dev report's own exact required scenario:
`NEW -> PREPARING -> READY -> POS qty increase -> delta production ->
READY`, confirming the original quantity is never re-prepared and no
production history (state, qty, timestamps) is lost at any point.

**Total: 225 tests** (up from 224).

No database migration required. No production code touched outside
`models/pos_order.py`, `models/kds_order_line.py` (docstring only) -
this is a backend/workflow-layer fix, not a frontend display change
(the `UPDATED (+N)` badge display itself was already correct, per the
dev report's own confirmation).
