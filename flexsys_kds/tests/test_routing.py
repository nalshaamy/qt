...
class TestRouting(FlexSysKdsTestCommon):
    ...
    # -----------------------------------------------------------------
    def test_v68_pos_field_label_is_pos_filter_optional(self):
        """RENAMED from test_patch5_item2_pos_field_label_simplified
        (stale, from the pre-V68 'POS' label contract - fixed on a
        fresh Odoo.sh repository validation run). The approved
        current UX ("Station / Routing UX Clarification") is 'POS
        Filter (Optional)', already covered in more detail by
        test_routing_rule_pos_field_label_updated below - this test
        is kept (renamed, not duplicated) specifically for its own
        non-regression check that empty pos_config_ids still matches
        any POS, immediately following it."""
        field = self.env['kds.routing.rule']._fields['pos_config_ids']
        self.assertEqual(field.string, 'POS Filter (Optional)')
        self.assertIn('Leave empty', field.help)
        self.assertIn('any POS already allowed by the destination station', field.help)
    ...
    def test_v68_routing_logic_help_text_updated(self):
        """RENAMED from test_patch5_item4_matching_help_simplified
        (stale, still expected the pre-V68 wording "Empty criterion =
        Any" - fixed on a fresh Odoo.sh repository validation run).
        The approved current wording ("Station / Routing UX
        Clarification", item 7) replaces that phrasing entirely -
        deliberately not restoring the old wording just to satisfy
        this test.

        Whitespace is normalized before each assertion: arch_db
        stores the view's own raw XML source text exactly as written,
        including the line-wrapping and indentation of this file's
        own <div class="alert"> block - a full-sentence assertion
        checked directly against that raw text (as the ORIGINAL,
        now-stale version of this test did) would itself be fragile
        to nothing more than how a sentence happens to be word-wrapped
        across source lines, independent of whether the actual wording
        is correct - exactly how the previous version of this test
        broke in the first place."""
        import re
        form_view = self.env.ref('flexsys_kds.view_kds_routing_rule_form')
        arch = re.sub(r'\s+', ' ', form_view.arch_db)
        self.assertIn('Routing logic:', arch)
        self.assertIn('Empty filters match any value.', arch)
        self.assertIn('Multiple values', arch)
        self.assertIn('use OR', arch)
        self.assertIn('Different filters', arch)
        self.assertIn('using AND', arch)
        self.assertIn('Station POS restrictions are applied before routing filters.', arch)
...
@tagged('post_install', '-at_install')
class TestRoutingUxClarification(FlexSysKdsTestCommon):
    """UX CLARIFICATION ("Station / Routing UX Clarification") - the
    contradictory-configuration prevention (item 5) is the only part
    of this change with genuine new backend behavior; everything else
    (labels/help text, section placement, the informational Allowed
    POS display, the rule summary) is presentation-only and has no
    runtime behavior of its own to regression-test here. Routing
    evaluation semantics themselves (_matches()/route_product()) are
    completely unchanged and continue to be covered by the rest of
    this file."""

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.pos_restaurant = cls._make_test_pos_config('Restaurant')
        cls.pos_bakery = cls._make_test_pos_config('Bakery Shop')

    def test_1_station_restricted_and_empty_rule_filter_is_valid(self):
        """Destination Station allows Restaurant + POS Filter empty
        => valid."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station A', 'code': 'UXTESTA', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule 1',
            'station_id': station.id,
        })
        self.assertTrue(rule.exists(), "An empty POS Filter must always be valid, regardless of station restrictions.")

    def test_2_station_restricted_and_matching_rule_filter_is_valid(self):
        """Destination Station allows Restaurant + POS Filter
        Restaurant => valid."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station B', 'code': 'UXTESTB', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule 2',
        ...
        self.assertTrue(rule.exists(), "A POS Filter matching the station's own allowed set must be valid.")

    def test_3_station_restricted_and_disallowed_rule_filter_is_rejected(self):
        """Destination Station allows Restaurant + POS Filter Bakery
        Shop => rejected, with the exact message format requested."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station C', 'code': 'UXTESTC', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        with self.assertRaises(ValidationError) as cm:
            self.env['kds.routing.rule'].create({
                'name': 'UX Test Rule 3',
                'station_id': station.id,
                'pos_config_ids': [(6, 0, [self.pos_bakery.id])],
            })
        self.assertIn('Bakery Shop', str(cm.exception))
        self.assertIn('not allowed by the selected destination station', str(cm.exception))

    def test_4_station_allows_all_and_any_rule_filter_is_valid(self):
        """Station Allowed POS empty + any POS Filter => valid, because
        the station allows all."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station D', 'code': 'UXTESTD', 'target_prep_time': 10,
            # pos_config_ids deliberately left empty - station allows all.
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule 4',
            'station_id': station.id,
        ...
        self.assertTrue(
            rule.exists(),
            "A POS Filter must never be rejected when the station itself allows all POS."
        )

    def test_5_changing_destination_station_recomputes_pos_filter_domain(self):
        """Changing Destination Station must recompute available POS
        Filter choices - pos_filter_domain_ids is the compute backing
        that UI domain, so this verifies it recalculates correctly
        when station_id changes, both in and out of a restriction."""
        station_restricted = self.env['kds.station'].create({
            'name': 'UX Test Station E1', 'code': 'UXTESTE1', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        station_open = self.env['kds.station'].create({
            'name': 'UX Test Station E2', 'code': 'UXTESTE2', 'target_prep_time': 10,
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule 5',
            'station_id': station_restricted.id,
        })
        self.assertEqual(
            set(rule.pos_filter_domain_ids.ids), {self.pos_restaurant.id},
            "The domain must be scoped to exactly the restricted station's own allowed POS."
        )
        rule.station_id = station_open.id
        rule.invalidate_recordset(['pos_filter_domain_ids'])
        all_pos_ids = set(self.env['pos.config'].search([]).ids)
        self.assertEqual(
            set(rule.pos_filter_domain_ids.ids), all_pos_ids,
            "Once the station allows all POS, the domain must offer every existing POS config, "
            "not an empty (and therefore misleadingly restrictive-looking) set."
        )

    def test_6_existing_empty_pos_filter_behavior_unchanged(self):
        """Existing Routing behavior with an empty POS filter remains
        unchanged - route_product()/_matches() themselves are
        untouched by this whole change; this is a direct regression
        check against the pre-existing routing engine, not the new
        UX-only fields."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station F', 'code': 'UXTESTF', 'target_prep_time': 10,
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule 6',
            'product_ids': [(6, 0, [self.product_burger.id])],
        ...
        self.assertTrue(
            rule._matches(self.product_burger, 'dine_in', 'pos', self.pos_restaurant),
            "An empty pos_config_ids on the rule must still match any POS, exactly as before."
        )
        self.assertTrue(
            rule._matches(self.product_burger, 'dine_in', 'pos', self.pos_bakery),
            "An empty pos_config_ids on the rule must still match any POS, exactly as before."
        )

    def test_redundant_pos_filter_onchange_warns_without_blocking(self):
        """Item 6: functionally valid but redundant configuration must
        only warn, never block - and must NOT warn for a genuinely
        meaningful partial filter on a multi-POS station."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station G', 'code': 'UXTESTG', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        rule = self.env['kds.routing.rule'].new({
            'name': 'UX Test Rule 7',
            'station_id': station.id,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        result = rule._onchange_pos_filter_redundant_warning()
        self.assertIsNotNone(result, "An exact-match POS Filter must trigger the redundant warning.")
        self.assertIn('warning', result)
        self.assertIn('Restaurant', result['warning']['message'])

        multi_station = self.env['kds.station'].create({
            'name': 'UX Test Station H', 'code': 'UXTESTH', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id, self.pos_bakery.id])],
        })
        partial_rule = self.env['kds.routing.rule'].new({
            'name': 'UX Test Rule 8',
            'station_id': multi_station.id,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        partial_result = partial_rule._onchange_pos_filter_redundant_warning()
        self.assertIsNone(
            partial_result,
            "A POS Filter that meaningfully narrows a multi-POS station's own allowed set "
            "must not be flagged as redundant."
        )

    def test_rule_summary_reflects_configuration(self):
        """Item 8: optional rule summary - purely informational,
        computed from already-loaded fields, no bearing on actual
        routing evaluation."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station I', 'code': 'UXTESTI', 'target_prep_time': 10,
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule 9',
            'product_ids': [(6, 0, [self.product_burger.id])],
        ...
        self.assertIn(station.name, rule.rule_summary)
        self.assertIn('any allowed POS', rule.rule_summary)

    def test_station_help_text_updated(self):
        """Item 1: the station's own field label/help text, verified
        directly on the field definition."""
        field = self.env['kds.station']._fields['pos_config_ids']
        self.assertEqual(field.string, 'Allowed POS')
        self.assertIn('Select which POS systems can send orders to this station', field.help)
        self.assertIn('Leave empty to allow all POS', field.help)

    def test_routing_rule_pos_field_label_updated(self):
        """Item 2: the routing rule's own field label/help text,
        verified directly on the field definition."""
        field = self.env['kds.routing.rule']._fields['pos_config_ids']
        self.assertEqual(field.string, 'POS Filter (Optional)')
        self.assertIn('Optional', field.help)
        self.assertIn('any POS already allowed by the destination station', field.help)

    # -----------------------------------------------------------------
    # V68 REVIEW ("Station-Side Contradiction Validation") - the other
    # half of the contradiction-prevention requirement: editing the
    # STATION's own Allowed POS down to something narrower than what
    # an existing routing rule already targeting it relies on.
    # -----------------------------------------------------------------

    def test_a_editing_station_to_remove_pos_a_rule_relies_on_is_rejected(self):
        """A) Station allows Restaurant+Bakery, Rule filters Bakery,
        change Station to Restaurant only => REJECTED, naming both the
        offending rule and the disallowed POS explicitly."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station J', 'code': 'UXTESTJ', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id, self.pos_bakery.id])],
        })
        self.env['kds.routing.rule'].create({
            'name': 'Dessert Route',
        ...
        with self.assertRaises(ValidationError) as cm:
            station.pos_config_ids = [(6, 0, [self.pos_restaurant.id])]
        self.assertIn('Dessert Route', str(cm.exception))
        self.assertIn('Bakery Shop', str(cm.exception))

    def test_b_editing_station_to_allow_all_is_always_valid(self):
        """B) Station allows Restaurant+Bakery, Rule filters Bakery,
        change Station Allowed POS to empty (Allow All) => VALID -
        emptying a station's own restriction can never contradict any
        existing rule, by definition."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station K', 'code': 'UXTESTK', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id, self.pos_bakery.id])],
        })
        self.env['kds.routing.rule'].create({
            'name': 'Dessert Route 2',
            'station_id': station.id,
            'pos_config_ids': [(6, 0, [self.pos_bakery.id])],
        })
        station.pos_config_ids = [(5, 0, 0)]
        self.assertFalse(station.pos_config_ids, "Station must now allow all POS.")

    def test_c_editing_station_while_still_covering_rules_own_filter_is_valid(self):
        """C) Station allows Restaurant+Bakery, Rule filters
        Restaurant, change Station to Restaurant only => VALID - the
        rule's own filter is still fully covered by the station's new,
        narrower Allowed POS."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station L', 'code': 'UXTESTL', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id, self.pos_bakery.id])],
        })
        self.env['kds.routing.rule'].create({
            'name': 'Restaurant Route 2',
            'station_id': station.id,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id])],
        })
        station.pos_config_ids = [(6, 0, [self.pos_restaurant.id])]
        self.assertEqual(station.pos_config_ids, self.pos_restaurant)

    # -----------------------------------------------------------------
    # V68 REVIEW ("Allowed POS Display When Station Allows All")
    # -----------------------------------------------------------------

    def test_station_pos_display_shows_all_pos_text_when_unrestricted(self):
        """When the station allows all POS (empty pos_config_ids), the
        display field must show the explicit "All POS" text - never a
        blank value that could be misread as "no POS allowed"."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station M', 'code': 'UXTESTM', 'target_prep_time': 10,
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule M',
            'station_id': station.id,
        })
        self.assertEqual(rule.station_pos_config_display, 'All POS')

    def test_station_pos_display_shows_names_when_restricted(self):
        """When the station restricts to specific POS, the display
        field must show their actual names."""
        station = self.env['kds.station'].create({
            'name': 'UX Test Station N', 'code': 'UXTESTN', 'target_prep_time': 10,
            'pos_config_ids': [(6, 0, [self.pos_restaurant.id, self.pos_bakery.id])],
        })
        rule = self.env['kds.routing.rule'].create({
            'name': 'UX Test Rule N',
        ...
        self.assertIn('Restaurant', rule.station_pos_config_display)
        self.assertIn('Bakery Shop', rule.station_pos_config_display)
        self.assertNotEqual(rule.station_pos_config_display, 'All POS')
