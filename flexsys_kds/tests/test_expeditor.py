...
class TestExpeditor(FlexSysKdsTestCommon):
    ...
    def _order_two_stations(self):
        ...
        order.line_ids[0].write({'station_id': self.station_kitchen.id})
        order.line_ids[1].write({'station_id': self.station_coffee.id})
        return order

    def _order(self):
        order = self._make_order([(self.product_burger, 1)]).with_user(self.admin)
