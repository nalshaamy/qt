...
class TestPermissions(FlexSysKdsTestCommon):
    ...
    def test_station_with_no_users_is_invisible_to_random_operator(self):
        ...
        order.line_ids.write({'station_id': self.station_coffee.id})
    ...
    def test_operator_search_scoped_to_own_station_only(self):
        ...
        coffee_order.line_ids.write({'station_id': self.station_coffee.id})
    ...
    def test_bypass_check_skips_permission_for_internal_calls(self):
        ...
        # Same runtime field-name detection as common.py's
        # _make_kds_user() - see hooks.py's docstring for the full
        # history of why this can't be hardcoded here.
        groups_field = next(
        ...
        plain_user = self.env['res.users'].with_context(no_reset_password=True).create({
            'name': 'No KDS Role',
            'login': 'no_kds_role',
            'email': 'no_kds_role@example.com',
            groups_field: [(6, 0, [self.env.ref('base.group_user').id])],
        })
        order = self._order_at_kitchen()
        line = order.line_ids.with_user(plain_user)
        # Without bypass, a user with zero FlexSys KDS groups is denied:
        with self.assertRaises(AccessError):
