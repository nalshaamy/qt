...
class TestPermissions(FlexSysKdsTestCommon):
    ...
    def test_read_group_also_respects_station_scope(self):
        ...
        coffee_order = self._make_order([(self.product_cappuccino, 1)])
        self._route_line_to_station(coffee_order.line_ids, self.station_coffee)
        groups = self.env['kds.order.line'].with_user(user).read_group(
            [], ['station_id'], ['station_id'])
        seen_station_ids = {g['station_id'][0] for g in groups if g.get('station_id')}
    ...
    def test_bypass_check_skips_permission_for_internal_calls(self):
        """The delta-sync engine (and other internal/system flows) must be
        able to act without an interactive user's permissions - this is
        what bypass_check=True is for, and it should never be reachable
        from the controller (see controllers/kds.py, which never passes it).

        REAL BUG FIX, confirmed live on Odoo.sh: this used to test with a
        user holding zero FlexSys KDS groups at all - but bypass_check
        only skips the KDS-*specific* action/station-tier check
        (_kds_check_action's own `if bypass: return`), never Odoo's own
        base model access rights (ir.model.access.csv). A user with no
        KDS group has no *base* read/write access to kds.order.line at
        all, so merely fetching line.state (needed internally by
        _line_transition, before bypass_check is ever consulted) already
        raised AccessError regardless of the bypass flag - the test was
        never actually exercising bypass_check's own behavior. Fixed
        with a scenario that genuinely demonstrates it: a real Operator
        (has base model access, has the KDS group) assigned to a
        *different* station than the line being acted on - matching how
        bypass_check is actually used in production (an internal system
        flow like POS Delta Sync has no "station" of its own to be
        scoped to, and needs to act across every station regardless).
        """
        wrong_station_operator = self._make_kds_user(
            'wrong_station_op', self.group_operator, self.station_coffee)
        order = self._order_at_kitchen()
        line = order.line_ids.with_user(wrong_station_operator)
        # Without bypass, an Operator with no assignment to THIS line's
        # station is denied (the station-tier check, not the base ACL -
        # they do have base read/write access via group_kds_operator):
        with self.assertRaises(AccessError):
