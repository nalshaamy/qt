...
class TestPhase2DirectPrintingHttp(HttpCase):
    ...
    def test_http_prepare_invalid_token_is_rejected(self):
        ...
        # Warnings"): this call deliberately triggers
        # _station_from_token()'s own genuine, intentional
        # WARNING-level "token mismatch" rejection log - expected,
        # security-relevant production behavior this test exists to
        # prove, not a sign anything is actually wrong. Scoped
        # narrowly to only this one call, only this one logger name.
