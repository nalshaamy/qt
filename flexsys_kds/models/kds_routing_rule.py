...
class KdsRoutingRule(models.Model):
    ...
    def _matches(self, product, order_type, source, pos_config):
        ...
        # incoming POS config is absent or different - fixed by dropping
        # the truthy guard entirely.
        if self.pos_config_ids and pos_config not in self.pos_config_ids:
            return False
        return True

    @api.model
    def _station_eligible(self, station, company, pos_config):
        ...
        if not station:
            return False
        if company and station.company_id != company:
            return False
    ...
    def route_product(self, product, order_type='dine_in', source='pos', pos_config=None, company=None):
        ...
        for rule in rules:
            if rule._matches(product, order_type, source, pos_config) \
                    and self._station_eligible(rule.station_id, company, pos_config):
