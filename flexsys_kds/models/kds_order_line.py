...
class KdsOrderLine(models.Model):
    ...
    @api.model
    def normalize_note_text(self, raw_note):
        """Normalize a POS line note for display, wherever it is
        rendered (Internal KDS, Public Kiosk, the shared ticket/
        printing renderer).

        BUG FIX ("POS Line Note displayed as raw JSON in KDS"):
        confirmed on real Odoo 19 runtime - a line's own 'note' value
        can arrive as a JSON-serialized list of structured note
        objects (e.g. '[{"text":"Heating","colorIndex":0}]') rather
        than a plain string, produced by Odoo 19's own POS Quick/
        Customer Notes feature (color-coded tags) - the raw field is
        still a plain Char at the ORM level, but its own stored
        VALUE is a JSON-serialized structure rather than free text.
        Reading it with plain getattr()/display, as every caller of
        this note previously did, showed the JSON syntax itself to
        the kitchen ("[{"text":"Heating","colorIndex":0}]") instead
        of the human-readable word ("Heating").

        Returns only the human-readable text - never JSON brackets,
        object keys, or metadata like colorIndex - joined with ", "
        when multiple note entries exist. A genuinely plain-text note
        (the overwhelming majority of existing/historical notes) is
        returned completely unchanged, byte for byte - this function
        only ever transforms a value that actually parses as the
        expected JSON note-array/object shape; anything else,
        including JSON that doesn't match that shape at all, is
        returned as-is rather than guessed at or silently dropped.
        """
        if not raw_note:
            return raw_note or False
        if not isinstance(raw_note, str):
            # Defensive only - the ORM's own Char field always yields
            # a str or falsy value; this covers a caller passing an
            # already-parsed Python structure directly.
            return self._extract_note_texts_or_none(raw_note) or raw_note
        stripped = raw_note.strip()
        if not stripped or stripped[0] not in ('[', '{'):
            # Not JSON-shaped at all - a genuinely plain-text note,
            # returned completely unchanged for full compatibility
            # with every existing plain-text note.
            return raw_note
        try:
            parsed = json.loads(stripped)
        except (ValueError, TypeError):
            # Looks JSON-shaped (starts with [ or {) but doesn't
            # actually parse - safer to treat as plain text than to
            # guess or raise.
            return raw_note
        texts = self._extract_note_texts_or_none(parsed)
        if texts is None:
            # Valid JSON, but not the expected note-object shape at
            # all (e.g. an unrelated JSON structure) - fall back to
            # the original raw text rather than silently dropping
            # potentially meaningful content.
            return raw_note
        return ', '.join(texts) if texts else False

    @api.model
    def _extract_note_texts_or_none(self, parsed):
        """Returns a list of human-readable text strings extracted
        from a parsed JSON note structure (a list of {"text": ...}
        objects, a single such object, or a list/mix of plain
        strings), or None if nothing in `parsed` matches any
        recognized note shape at all. Unrecognized individual entries
        within an otherwise-recognized list are skipped rather than
        invalidating the whole list, since a single malformed entry
        should not hide every other genuinely readable note."""
        ...
        return texts if recognized_any else None
    ...
    # CHANGE REQUEST FIX ("POS Send-to-KDS Settings... Please verify
    # this against real Odoo 19 POS behavior or redesign the removal
    # sync so it cannot leak early"), confirmed live: pos_order_line.py's
    # own unlink() used to cancel this line immediately, resting on an
    # unverified assumption that a backend unlink() call only ever
    # happens as part of the same batched request that also carries the
    # Send/New signal. Redesigned so correctness no longer depends on
    # that assumption at all: unlink() now only sets this flag (a plain
    # field write, touching neither `state` nor anything the KDS screens
    # actually render), preserving the fact that removal happened
    # without making it visible yet. The line's own state, and
    # everything about how it looks on screen, is completely untouched
    # until _flexsys_kds_diff_lines() - which only ever runs at the
    # correct sync boundary for whichever trigger mode is configured -
    # actually processes it (see that method's own handling of this
    # flag in pos_order.py) and applies the real, audited cancellation
    # at that point, never before.
    pending_removal = fields.Boolean(default=False)
    ...
    def _system_cancel_after_completion(self, reason=False):
        """REAL BUG FIX ("Change Request After BUG-11", item 1: "Completed
        Order - Deleted POS Line Must Become CANCELLED"), confirmed live:
        a cashier deleting a previously-completed product from an
        already-COMPLETED POS order left the corresponding KDS line
        displayed as if it were normally completed - action_cancel()
        itself explicitly refuses to cancel a Completed line (a correct,
        deliberate restriction for an OPERATOR trying to cancel a dish
        that was genuinely already served), but this is a different,
        system-driven scenario: the cashier removed the item entirely, so
        marking it Cancelled is exactly the correct, intended new
        outcome, not something to block.

        Internal-only (not exposed via any UI action or bypass_check
        parameter - the "cannot cancel a Completed line" restriction
        stays fully intact for every real user-facing path; this exists
        specifically for POS Delta Sync's own line-removal detection in
        pos_order.py, and only for a line already sitting at 'completed'
        at the moment its POS line disappears). Mirrors action_cancel()'s
        own audit/notification/field-writing behavior exactly, so a
        Completed-then-removed line looks identical on screen to any
        other cancellation (same CANCELLED badge, same struck-through
        treatment, same retention window) - "the cancelled line should
        remain visible inside the completed card according to the
        existing retention/history behavior."

        Deliberately does NOT touch the order's own state, and does not
        call _system_reopen_if_production_incomplete() itself (the
        caller in pos_order.py already does, unconditionally, for every
        kind of line removal) - "do not reopen the whole order because a
        previously completed line was deleted... there is no new
        production work in this scenario." This is naturally satisfied
        without any special-casing: is_expeditor_ready (which that
        reopen check depends on) already excludes cancelled lines, so
        cancelling one Completed line while its Completed siblings remain
        Completed still correctly reads as "every remaining line is
        done" - the order stays exactly where it was.
        """
        for line in self:
            ...
            self.env['kds.event'].log(
                line.order_id, event_type='line_removed', station=line.station_id,
                old_value=previous_state, new_value='cancelled',
                note=_("Cancelled: %(qty)s x %(product)s - %(reason)s (removed from POS "
                       "after the original line was already completed - order remains "
                       "Completed, no production reopened)") % {
                    'qty': line.qty,
                    'product': line.product_name,
                    'reason': reason or _('no reason given'),
                })
