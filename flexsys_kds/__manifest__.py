...
- Printing: Direct Network (Epson ePOS) browser printing, with
  Manual Direct Print and POS Direct Auto Print, all tracked through
  the same centralized `kds.print.job` record.
...
See README.md for full product documentation and docs/ARCHITECTURE.md
for technical/integration detail.
...
Technical module name: flexsys_kds
    ...
    'license': 'OPL-1',
    'depends': ['base', 'web', 'point_of_sale', 'pos_restaurant', 'product'],
    'data': [
    ...
    'assets': {
        'web.assets_backend': [
            ...
            # Shared ticket renderer - loaded before kds_app.js so
            # window.FlexSysTicketBuilder is defined before it is used.
            'flexsys_kds/static/src/shared/flexsys_ticket_renderer.js',
            # Adaptive pagination - loaded before kds_app.js so
            # window.FlexSysPagination is defined before it is used.
        ...
        # Offline-send warning: a minimal patch on Odoo's own
        # sendOrderInPreparation() hook that persists a "Pending
        # Kitchen Send" warning in browser localStorage if a Send
        # fails offline, and re-shows it on reconnect.
        #
        # POS Direct Auto Print Worker: the shared ticket renderer and
        # Direct ePOS adapter must load before the worker itself,
        # which depends on both. Only these three POS-side files are
        # loaded here; the Public Kiosk's own standalone JS is not
        # part of the POS bundle.
