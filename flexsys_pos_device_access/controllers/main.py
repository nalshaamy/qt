...
class FlexSysPosDeviceAccessController(http.Controller):
    ...
    def _database_name(self):
        """Return the database selected for the current request."""
        return request.db or request.env.cr.dbname

    def _database_query(self):
        return urlencode({"db": self._database_name()})

    def _device_cookie_name(self):
        """Namespace the browser credential per database on shared hosts."""
        dbname = self._database_name() or ""
        suffix = hashlib.sha256(dbname.encode("utf-8")).hexdigest()[:12]
        return f"{_DEVICE_COOKIE}_{suffix}"
    ...
    def _response(self, body, status=403, clear_cookie=False):
        ...
        if clear_cookie:
            response.delete_cookie(self._device_cookie_name(), path="/")
            # Compatibility cleanup for pre-multi-database builds.
    ...
    def _establish_device_session(self, device):
        ...
        # A Linked POS User may belong to several companies while their normal
        # Odoo session currently points to another one. The POS data loader
        # applies company record rules. If the assigned device company is not
        # the active session company, Odoo can return an empty pos.config and
        # the POS frontend crashes while reading config.currency_id.
        #
        # Scope this device-bound session to the company of the assigned POS.
        # This changes only the generated device session; it does not change
        # the user's default company in res.users.
        company_id = device.company_id.id
        session_context = dict(session.context or {})
        session_context["allowed_company_ids"] = [company_id]
        session_context["company_id"] = company_id
        session["context"] = session_context

        session["flexsys_pos_device_id"] = device.id
        session["flexsys_pos_device_credential_version"] = device.credential_version
        session.touch()
        request.update_env(user=session.uid, context=session_context, su=False)
    ...
    def _set_device_cookie(self, response, raw_credential, device):
        ...
        # A previous development build used one global cookie name. Remove it
        # so two databases on the same hostname can never overwrite each other.
        response.delete_cookie(_DEVICE_COOKIE, path="/")
        return response
    ...
    def pair_device(self, token, **kwargs):
        ...
        response = request.redirect(f"/flexsys/pos/device/start?{self._database_query()}", code=303)
        response.headers["Cache-Control"] = "no-store"
        response.headers["Referrer-Policy"] = "no-referrer"
        return self._set_device_cookie(response, raw_credential, device)
    ...
    def device_start(self, **kwargs):
        ip_address = self._client_ip()
        raw_credential = (
            request.httprequest.cookies.get(self._device_cookie_name(), "")
            or request.httprequest.cookies.get(_DEVICE_COOKIE, "")
        )
        ...
        response = request.redirect(f"/pos/ui/{device.pos_config_id.id}/login?from_backend=1&flexsys_device_entry=1", code=303)
        response.headers["Cache-Control"] = "no-store"
        response.headers["Referrer-Policy"] = "no-referrer"
        return self._refresh_device_cookie(response, raw_credential, device)

    @http.route(
        "/flexsys/pos/device/session-info",
        type="jsonrpc",
        auth="user",
        methods=["POST"],
        readonly=False,
    ...
    def device_session_info(self):
        """Tell the POS UI whether this authenticated session is device-bound.

        This endpoint is UI metadata only. Backend confinement remains enforced
        server-side by ``backend_guard.py``.
        """
        device_id = request.session.get("flexsys_pos_device_id")
        version = request.session.get("flexsys_pos_device_credential_version")
        if not device_id:
            return {"is_device_session": False}
        ...
        valid = bool(
            ...
            and device.service_user_id.id == request.env.user.id
            and version == device.credential_version
        )
        return {"is_device_session": valid}

    @http.route(
        [
            "/flexsys/pos/device/cashier-login",
            # Backward-compatible endpoint for already cached POS assets.
    ...
    def cashier_login_reported(self, employee_id=None):
        device_id = request.session.get("flexsys_pos_device_id")
        version = request.session.get("flexsys_pos_device_credential_version")
        ...
        self._log(device=device, employee=employee, success=True, event="cashier_login_reported")
        return True
