...
class FlexSysPosDevice(models.Model):
    ...
    service_user_id = fields.Many2one(
        "res.users",
        string="Linked POS User",
        required=True,
        index=True,
        domain="[('active', '=', True), ('share', '=', False)]",
        help=(
            "Existing internal Odoo user used to establish the device POS session. "
            "FlexSys never creates an Odoo user automatically. "
    ...
    last_user_pin_success = fields.Char(
        string="Last Cashier Login",
        copy=False,
        readonly=True,
        help="Last cashier login reported by the native Odoo POS flow. FlexSys never receives or stores the PIN value.",
    )
    last_user_pin_success_at = fields.Datetime(
        string="Last Cashier Login At", copy=False, readonly=True
    )
    ...
    def _check_service_user_company(self):
        for rec in self:
            ...
            if not user:
                raise ValidationError(_("Select an existing Linked POS User."))
            if rec.company_id not in user.company_ids:
                raise ValidationError(_("The Linked POS User must have access to the device company."))
            if user._is_system():
                raise ValidationError(_("A system administrator cannot be used as the Linked POS User."))
            if not (
                user.has_group("point_of_sale.group_pos_user")
                or user.has_group("point_of_sale.group_pos_manager")
            ):
                raise ValidationError(_("The Linked POS User must have Point of Sale access (User or Administrator)."))
    ...
    def _service_user_is_valid(self):
        ...
        # Validate the actual pos.config payload that Odoo's POS loader will
        # request. A simple read(["id", "currency_id"]) is not sufficient:
        # another field/computation added by Odoo or a third-party module may
        # still raise AccessError and Odoo then returns pos.config = [], which
        # makes the frontend fail while dereferencing config.currency_id.
        ...
        try:
            records = config.env["pos.config"]._load_pos_data_search_read({}, config)
        except AccessError:
            return False
        return bool(records and records[0].get("id") == self.pos_config_id.id)
    ...
    def _build_pairing_wizard(self, raw_token):
        ...
        return {
            ...
            "view_mode": "form",
            "target": "new",
            # Deliberately keep the raw pairing URL out of every database
            # model, including TransientModel rows. It lives only in this
            # one response context until the modal is closed.
            "context": dict(self.env.context, flexsys_pairing_url=url),
        }
    ...
    def action_generate_pairing_link(self):
        ...
        if not self._service_user_is_valid():
            raise ValidationError(_("The Linked POS User is not valid. Select an existing internal user with Point of Sale access, then run Test Device Access."))
    ...
    def action_test_device_access(self):
        ...
        if not self._service_user_is_valid():
            problems.append(_(
                "Linked POS User cannot load the complete assigned Point of Sale data. "
                "Verify the user's company access, Point of Sale permissions, and any custom module access rules."
            ))
        try:
