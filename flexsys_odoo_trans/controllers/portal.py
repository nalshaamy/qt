...
SESSION_KEY = 'flexsys_inventory_manager_id'
LEGACY_SESSION_KEY = 'flexsys_trans_manager_id'
...
class FlexsysTransPortal(http.Controller):
    ...
    def _current_manager(self):
        manager_id = request.session.get(SESSION_KEY) or request.session.get(LEGACY_SESSION_KEY)
        if not manager_id:
    ...
    def _picking_domain(self, locations, extra=None):
        domain = ['|', ('location_id', 'in', locations.ids), ('location_dest_id', 'in', locations.ids)]
        if extra:
            domain += extra
        return domain

    def _operation_type(self, picking):
        ...
        if code == 'internal':
            return 'تحويل داخلي'
        return picking.picking_type_id.name or 'عملية مخزنية'

    def _activity_rows(self, pickings):
        rows = []
        for picking in pickings:
            rows.append({
                'id': picking.id,
                'name': picking.name,
                'kind': self._operation_type(picking),
                'date': picking.date_done or picking.scheduled_date or picking.create_date,
    ...
    def dashboard(self, manager, **kwargs):
        ...
        quants = quant_model.search([('location_id', 'in', locations.ids)])
        ...
        base_domain = self._picking_domain(locations)
        pending_pickings = picking_model.search_count(base_domain + [('state', 'not in', ['done', 'cancel'])])
        internal_transfers = picking_model.search_count(base_domain + [('picking_type_id.code', '=', 'internal'), ('state', 'not in', ['done', 'cancel'])])
        incoming_today = picking_model.search_count(base_domain + [('picking_type_id.code', '=', 'incoming'), ('scheduled_date', '>=', fields.Date.today())])
        outgoing_today = picking_model.search_count(base_domain + [('picking_type_id.code', '=', 'outgoing'), ('scheduled_date', '>=', fields.Date.today())])
        count_open = quant_model.search_count([
            ('location_id', 'in', locations.ids),
            '|', ('inventory_quantity_set', '=', True), ('inventory_diff_quantity', '!=', 0),
        ])
        latest_pickings = picking_model.search(base_domain, order='write_date desc, id desc', limit=8)
        alerts = []
        if pending_pickings:
            alerts.append({'level': 'warning', 'title': 'عمليات تحتاج متابعة', 'text': f'{pending_pickings} عملية مخزنية لم تُغلق بعد', 'url': '/fstrans/dashboard/warehouses/operations'})
        if internal_transfers:
            alerts.append({'level': 'info', 'title': 'تحويلات نشطة', 'text': f'{internal_transfers} تحويل داخلي قيد التنفيذ', 'url': '/fstrans/dashboard/warehouses/transfers'})
        if out_of_stock:
            alerts.append({'level': 'danger', 'title': 'أرصدة تحتاج مراجعة', 'text': f'{out_of_stock} منتج دون رصيد متاح', 'url': '/fstrans/dashboard/warehouses/stock?balance=non_positive'})
        return request.render('flexsys_odoo_trans.dashboard_page', {
            ...
            'activities': self._activity_rows(latest_pickings),
            'alerts': alerts,
        })

    @http.route('/fstrans/dashboard/warehouses', type='http', auth='public', website=False, sitemap=False)
    @manager_required()
    def warehouses_overview(self, manager, **kwargs):
        locations = self._warehouse_locations(manager)
        quant_model = request.env['stock.quant'].sudo()
        picking_model = request.env['stock.picking'].sudo()
        cards = []
        for warehouse in manager.warehouse_ids:
            roots = warehouse.view_location_id
            wh_locations = request.env['stock.location'].sudo().search([
                ('id', 'child_of', roots.ids), ('usage', '=', 'internal'),
            ])
            quants = quant_model.search([('location_id', 'in', wh_locations.ids)])
            cards.append({
                'warehouse': warehouse,
                'product_count': len(set(quants.mapped('product_id').ids)),
                'available': sum(quants.mapped('quantity')) - sum(quants.mapped('reserved_quantity')),
                'pending': picking_model.search_count(self._picking_domain(wh_locations, [('state', 'not in', ['done', 'cancel'])])),
                'locations': len(wh_locations),
            })
        return request.render('flexsys_odoo_trans.warehouses_page', {
            'manager': manager, 'warehouse_label': self._warehouse_label(manager),
            'active_menu': 'warehouses', 'warehouse_cards': cards,
        })

    @http.route(['/fstrans/dashboard/stock', '/fstrans/dashboard/warehouses/stock'], type='http', auth='public', website=False, sitemap=False)
    ...
    def stock(self, manager, **kwargs):
        ...
        pager = self._pagination('/fstrans/dashboard/warehouses/stock', len(all_rows), page, page_size, kwargs)
    ...
    @http.route(['/fstrans/dashboard/movements', '/fstrans/dashboard/warehouses/movements'], type='http', auth='public', website=False, sitemap=False)
    ...
    def movements(self, manager, **kwargs):
        ...
        pager = self._pagination('/fstrans/dashboard/warehouses/movements', total, page, page_size, kwargs)
    ...
    def _operation_filters(self, kwargs):
        return {
    ...
    def _operation_domain(self, locations, filters, internal_only=False):
        domain = self._picking_domain(locations)
        if internal_only:
            domain.append(('picking_type_id.code', '=', 'internal'))
        elif filters.get('type') in ('incoming', 'outgoing', 'internal'):
            domain.append(('picking_type_id.code', '=', filters['type']))
        if filters.get('state'):
            domain.append(('state', '=', filters['state']))
        if filters.get('reference'):
            domain += ['|', ('name', 'ilike', filters['reference']), ('origin', 'ilike', filters['reference'])]
        if filters.get('date_from'):
            domain.append(('scheduled_date', '>=', filters['date_from']))
        if filters.get('date_to'):
            domain.append(('scheduled_date', '<=', filters['date_to'] + ' 23:59:59'))
        return domain

    @http.route('/fstrans/dashboard/warehouses/operations', type='http', auth='public', website=False, sitemap=False)
    @manager_required('can_view_moves')
    def operations(self, manager, **kwargs):
        locations = self._warehouse_locations(manager)
        filters = self._operation_filters(kwargs)
        page = self._positive_int(kwargs.get('page'), 1)
        page_size = self._page_size(kwargs.get('page_size'))
        model = request.env['stock.picking'].sudo()
        domain = self._operation_domain(locations, filters)
        total = model.search_count(domain)
        pager = self._pagination('/fstrans/dashboard/warehouses/operations', total, page, page_size, kwargs)
        pickings = model.search(domain, order='scheduled_date desc, id desc', limit=page_size, offset=(pager['page'] - 1) * page_size)
        return request.render('flexsys_odoo_trans.operations_page', {
            'manager': manager, 'warehouse_label': self._warehouse_label(manager),
            'active_menu': 'operations', 'pickings': pickings, 'filters': filters,
            'pager': pager,
        })

    @http.route('/fstrans/dashboard/warehouses/transfers', type='http', auth='public', website=False, sitemap=False)
    @manager_required('can_view_transfers')
    def transfers(self, manager, **kwargs):
        locations = self._warehouse_locations(manager)
        filters = self._operation_filters(kwargs)
        page = self._positive_int(kwargs.get('page'), 1)
        page_size = self._page_size(kwargs.get('page_size'))
        model = request.env['stock.picking'].sudo()
        domain = self._operation_domain(locations, filters, internal_only=True)
        total = model.search_count(domain)
        pager = self._pagination('/fstrans/dashboard/warehouses/transfers', total, page, page_size, kwargs)
        pickings = model.search(domain, order='scheduled_date desc, id desc', limit=page_size, offset=(pager['page'] - 1) * page_size)
        return request.render('flexsys_odoo_trans.transfers_page', {
            'manager': manager, 'warehouse_label': self._warehouse_label(manager),
            'active_menu': 'transfers', 'pickings': pickings, 'filters': filters,
            'pager': pager,
        })

    @http.route('/fstrans/dashboard/warehouses/inventory-count', type='http', auth='public', website=False, sitemap=False)
    @manager_required('can_view_stock')
    def inventory_count(self, manager, **kwargs):
        locations = self._warehouse_locations(manager)
        page = self._positive_int(kwargs.get('page'), 1)
        page_size = self._page_size(kwargs.get('page_size'))
        ...
        if status == 'difference':
            domain.append(('inventory_diff_quantity', '!=', 0))
        elif status == 'open':
            domain.append(('inventory_quantity_set', '=', True))
        model = request.env['stock.quant'].sudo()
        total = model.search_count(domain)
        pager = self._pagination('/fstrans/dashboard/warehouses/inventory-count', total, page, page_size, kwargs)
        quants = model.search(domain, order='inventory_date desc, id desc', limit=page_size, offset=(pager['page'] - 1) * page_size)
        difference_count = model.search_count([('location_id', 'in', locations.ids), ('inventory_diff_quantity', '!=', 0)])
        open_count = model.search_count([('location_id', 'in', locations.ids), ('inventory_quantity_set', '=', True)])
        return request.render('flexsys_odoo_trans.inventory_count_page', {
            'manager': manager, 'warehouse_label': self._warehouse_label(manager),
            'active_menu': 'inventory_count', 'quants': quants, 'status': status,
            'difference_count': difference_count, 'open_count': open_count, 'pager': pager,
        })
